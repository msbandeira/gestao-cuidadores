<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar Plano de Serviço: ') }}<span class="text-indigo-600">{{ $servicePlan->name }}</span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('service_plans.update', $servicePlan->id) }}">
                        @csrf
                        @method('PUT') <div class="mb-4">
                            <x-input-label for="name" :value="__('Nome do Plano')" />
                            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $servicePlan->name)" required autofocus />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="description" :value="__('Descrição do Plano')" />
                            <textarea id="description" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" name="description" rows="3">{{ old('description', $servicePlan->description) }}</textarea>
                            <x-input-error :messages="$errors->get('description')" class="mt-2" />
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4"> {{-- Alterado para 3 colunas --}}
                            <div>
                                <x-input-label for="price" :value="__('Preço Total (R$)')" />
                                <x-text-input id="price" class="block mt-1 w-full" type="number" step="0.01" name="price" :value="old('price', $servicePlan->price)" required />
                                <x-input-error :messages="$errors->get('price')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="professional_repass_value" :value="__('Repasse p/ Profissional (R$)')" />
                                <x-text-input id="professional_repass_value" class="block mt-1 w-full" type="number" step="0.01" name="professional_repass_value" :value="old('professional_repass_value', $servicePlan->professional_repass_value)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="company_intake_value" class="block mt-1 w-full" type="number" step="0.01" name="company_intake_value" :value="old('company_intake_value', $servicePlan->company_intake_value)" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <x-text-input id="plan_type" class="block mt-1 w-full" type="text" name="plan_type" :value="old('plan_type', $servicePlan->plan_type)" placeholder="Ex: Por Hora, Mensal, Avulso" />
                            ...
                            <div>
                                ...
                                <x-text-input id="hours_included" class="block mt-1 w-full" type="number" name="hours_included" :value="old('hours_included', $servicePlan->hours_included)" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="days_validity" class="block mt-1 w-full" type="number" name="days_validity" :value="old('days_validity', $servicePlan->days_validity)" />
                            <x-input-error :messages="$errors->get('days_validity')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <label for="is_active" class="inline-flex items-center">
                                <input id="is_active" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="is_active" value="1" {{ old('is_active', $servicePlan->is_active) ? 'checked' : '' }}>
