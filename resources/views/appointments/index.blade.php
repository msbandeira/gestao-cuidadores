                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($appointments as $appointment)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">{{ $appointment->professional->full_name }}</td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">R$ {{ number_format($appointment->price_charged, 2, ',', '.') }}</td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <a href="{{ route('appointments.show', $appointment->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-4">Ver Detalhes</a>
                                                <a href="{{ route('appointments.edit', $appointment->id) }}" class="text-blue-600 hover:text-blue-900 mr-4">Editar</a>
                                                @can('acessar-admin')
                                                <form action="{{ route('appointments.destroy', $appointment->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Tem certeza que deseja excluir este atendimento?');">
                                                    @csrf
                                                    @method('DELETE')
